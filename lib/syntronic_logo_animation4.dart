...
class _LogoPathAnimation4PageState extends State<LogoPathAnimation4Page> with SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    animation1 = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: controller, curve: const Interval(0, 0.7)));
    animation2 = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: controller, curve: const Interval(0, 0.9)));
      ...
      return Scaffold(
        body: Center(
          child: Stack(
            ...
            children: <Widget>[
              CustomPaint(painter: PathTestPainter(path[0])),
              CustomPaint(painter: PathTestPainter(path[1])),

//              Opacity(opacity: animation1.value <= 0.3 ? 2.5 * animation1.value : (animation1.value > 0.3 && animation1.value <= 0.7) ? 1.0 : 2.5 - (2.5 * animation1.value), child: CustomPaint(painter: PathTestPainter(path[0])),),
//              Opacity(opacity: animation2.value <= 0.3 ? 2.5 * animation2.value : (animation2.value > 0.3 && animation2.value <= 0.7) ? 1.0 : 2.5 - (2.5 * animation2.value), child: CustomPaint(painter: PathTestPainter(path[1])),),
            ],
          ),
        ),
      );
